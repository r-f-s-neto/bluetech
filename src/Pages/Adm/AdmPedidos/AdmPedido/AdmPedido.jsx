  ...
  React.useEffect(() => {
    ...
    try {
      localStorageData = JSON.parse(
        window.localStorage.getItem('admClickedPedido'),
      );
      setErroPedido(false);
    } catch {
      setErroPedido(true);
    ...
    <div className="AdmPedido">
      ...
      {error && (
        <Alert variant="danger">
          Não foi possível encontrar os produtos do pedido, tente mais tarde !
        </Alert>
      )}
      {erroPedido && (
        <Alert variant="danger">
          Não foi possível carregar os dados do pedido, tente mais tarde
        </Alert>
      )}
      <Table responsive>
        <thead>
        ...
        <tbody>
          {pedido?.products.map((produto) => {
            return (
              <tr
                className="tableBody"
                key={produto.productId + 'tablePedidos'}
              >
                {products ? (
                  <td>
                    {
                      products.find(
                        (element) => produto.productId === element.id,
                      ).name
                    }
                  </td>
                ) : (
