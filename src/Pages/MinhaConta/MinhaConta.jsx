...
import cartImg from '../../assets/MinhaConta-assets/icons8-carrinho-de-compras-100.png';
...
const MinhaConta = () => {
  ...
  React.useEffect(() => {
    try {
      setUserName(JSON.parse(window.localStorage.getItem('blueDataUser')).name.split(' ')[0])
    } catch {
      setError(true)
    }
  }, []);
  return (
    ...
    <div className='MinhaConta'>
      <aside className='MinhaConta__aside asideUsersData'>
        ...
        {error && <Alert variant='danger'>Não foi possível acessar os dados, tente relogar</Alert>}
        <div className='asideUsersData__options optionsUser'>
          <button className='optionsUser__item' onClick={() => {
            navigate('/minha-conta/editar');
          }} >
            <img src={editUserImg} alt="imagem padrão de usuario" />
            <p>Editar meu perfil</p>
          </button>
          <button className='optionsUser__item'>
        ...
        <div className='MinhaConta__content'><Outlet /></div>
    </div>
  )
}
