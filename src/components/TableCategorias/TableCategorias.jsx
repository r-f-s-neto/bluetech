...
const TableCategorias = () => {
  ...
  async function handleClickDelete (categoria) {
    ...
    try{
      const response = await fetch('https://e-commerce-api-bluetech-production.up.railway.app/category/'+categoria.id, {
        method:"DELETE",
        credentials: "include"
      })
      if(response.ok) {
  ...
  return (
    ...
    {sucessDelete&&<Alert variant='success'>Categoria deletada com sucesso</Alert>}
    <Table responsive>
      <thead>
          ...
          return (
            ...
            >
              ...
              <td className="TableProducts__Buttons">
                ...
                <button className={loadingDelete?"admProductDelete-active":""} onClick={() => {handleClickDelete(categoria)}}>excluir</button>
              </td>
            </tr>
          );
        })}
